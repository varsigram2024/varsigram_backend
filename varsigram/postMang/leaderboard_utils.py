...
def key_daily(metric: str, dt: datetime.date | None = None) -> str:
    dt = dt or datetime.utcnow().date()
    return f"leaderboard:{metric}:daily:{dt.isoformat()}"

def key_weekly(metric: str, dt: datetime.date | None = None) -> str:
    dt = dt or datetime.utcnow().date()
    y, w, _ = dt.isocalendar()
    return f"leaderboard:{metric}:weekly:{y}-W{w:02d}"

def key_monthly(metric: str, dt: datetime.date | None = None) -> str:
    dt = dt or datetime.utcnow().date()
    return f"leaderboard:{metric}:monthly:{dt.year}-{dt.month:02d}"

def period_keys(metric: str, dt: datetime.date | None = None) -> list:
